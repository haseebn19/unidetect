import {extractTextFromFile, extractTextFromPDF, extractTextFromDOCX} from './fileExtraction';
...
describe('fileExtraction', () => {
    describe('extractTextFromFile', () => {
        it('should handle empty files', async () => {
            ...
            await expect(extractTextFromFile(emptyFile)).rejects.toThrow('File is empty or invalid');
        });

        it('should handle large files', async () => {
            // Create a mock file larger than 50MB
            const largeFileSize = 51 * 1024 * 1024; // 51MB
            const largeFile = new File(['x'.repeat(largeFileSize)], 'large.txt', {type: 'text/plain'});

            await expect(extractTextFromFile(largeFile)).rejects.toThrow('File is too large. Please use files smaller than 50MB.');
        });

        it('should extract text from TXT files', async () => {
            const textContent = 'Hello world!\nThis is a test file.';
            const txtFile = new File([textContent], 'test.txt', {type: 'text/plain'});
        ...
        it('should extract text from MD files', async () => {
            ...
            const mdFile = new File([markdownContent], 'test.md', {type: 'text/markdown'});
            ...
            await expect(extractTextFromFile(emptyTextFile)).rejects.toThrow('File is empty or invalid');
        });

        it('should handle whitespace-only text files', async () => {
            const whitespaceFile = new File(['   \n\t  '], 'whitespace.txt', {type: 'text/plain'});

            await expect(extractTextFromFile(whitespaceFile)).rejects.toThrow('Text file appears to be empty');
        });

        it('should try to read unknown file types as text', async () => {
            const textContent = 'This is plain text content';
            const unknownFile = new File([textContent], 'test.unknown', {type: 'application/octet-stream'});
    ...
    describe('extractTextFromPDF', () => {
        ...
        it('should handle empty PDF files', async () => {
            const emptyPdfFile = new File([], 'empty.pdf', {type: 'application/pdf'});

            await expect(extractTextFromPDF(emptyPdfFile)).rejects.toThrow('PDF file appears to be empty or corrupted');
        });
        ...
        it('should extract text from valid PDF', async () => {
            ...
            mockPdfJs.getDocument.mockReturnValue(mockLoadingTask); const result = await extractTextFromPDF(pdfFile);
        ...
        it('should handle PDFs with no text content', async () => {
            ...
            await expect(extractTextFromPDF(pdfFile)).rejects.toThrow('No readable text content found in this PDF');
        });

        it('should handle PDF processing errors', async () => {
            const pdfContent = new ArrayBuffer(1024);
            const pdfFile = new File([pdfContent], 'corrupted.pdf', {type: 'application/pdf'});

            mockPdfJs.getDocument.mockImplementation(() => {
                throw new Error('PDF parsing failed');
            });

            await expect(extractTextFromPDF(pdfFile)).rejects.toThrow('PDF parsing failed');
        });
    });
    ...
    describe('extractTextFromDOCX', () => {
        ...
        it('should handle empty DOCX files', async () => {
            const emptyDocxFile = new File([], 'empty.docx', {type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'});

            await expect(extractTextFromDOCX(emptyDocxFile)).rejects.toThrow('DOCX file appears to be empty or corrupted');
        });

        it('should extract text from valid DOCX', async () => {
            const docxContent = new ArrayBuffer(1024);
            const docxFile = new File([docxContent], 'test.docx', {type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'});
        ...
        it('should handle DOCX files with no text content', async () => {
            const docxContent = new ArrayBuffer(1024);
            const docxFile = new File([docxContent], 'empty-content.docx', {type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'});

            mockMammoth.extractRawText.mockResolvedValue({
                value: '   \n\t  '
            });

            await expect(extractTextFromDOCX(docxFile)).rejects.toThrow('No readable text content found in this DOCX file');
        });

        it('should handle DOCX processing errors', async () => {
            const docxContent = new ArrayBuffer(1024);
            const docxFile = new File([docxContent], 'corrupted.docx', {type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'});

            mockMammoth.extractRawText.mockRejectedValue(new Error('DOCX parsing failed'));

            await expect(extractTextFromDOCX(docxFile)).rejects.toThrow('DOCX parsing failed');
        });
    });
    ...
    describe('error handling', () => {
        it('should handle null or undefined files', async () => {
            await expect(extractTextFromFile(null as any)).rejects.toThrow('File is empty or invalid');
            await expect(extractTextFromFile(undefined as any)).rejects.toThrow('File is empty or invalid');
        });
        ...
        it('should preserve original error messages from extraction functions', async () => {
            ...
            mockMammoth.extractRawText.mockRejectedValue(customError);

            await expect(extractTextFromFile(docxFile)).rejects.toThrow('Custom extraction error');
            ...
            await expect(extractTextFromFile(docxFile)).rejects.toThrow('Failed to process DOCX file');
        });
    });
});